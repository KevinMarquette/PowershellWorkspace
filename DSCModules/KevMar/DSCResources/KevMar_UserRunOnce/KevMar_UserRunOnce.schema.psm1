...
Configuration KevMar_UserRunOnce{
    param(
    $ID,
    $Ensure = "Present",
    $Version  = "1.0",
    ...
    Registry "KevMar_UserRunOnce_$($ID)_StubPath"
    {
        Ensure = $Ensure
        Key = $ActiveSetupKey
        ValueName = "StubPath"
    ...
    Registry "KevMar_UserRunOnce_$($ID)_Version"
    {
        Ensure = $Ensure
        Key = $ActiveSetupKey
        ValueName = "Version"
        ValueData = $ActiveSetupVersion
        DependsOn = "[Registry]KevMar_UserRunOnce_$($ID)_StubPath"
    }
}
